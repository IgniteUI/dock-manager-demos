...
const isVerbose =
  process.env.VERBOSE === "1" || process.env.DEBUG_PROXY === "1";
...
function readPortsMap(): Record<string, number> {
  const fromEnv = safeParseJSON<Record<string, number>>(
    process.env[ENV_MAP_VAR],
  );
  if (fromEnv) {
    console.log(
      `${LOG_PREFIX} Using projects port map from env ${ENV_MAP_VAR}`,
    );
    return fromEnv;
  }
  ...
  console.log(
    `${LOG_PREFIX} No explicit projects port map found (env/file). Using fallback allocation.`,
  );
  return {};
}
...
function alphabeticalIndexFallback(id: string): number {
  const sortedIds = [...projects.map((p) => p.id)].sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  return {
    ...
    configure: (proxy) => {
      proxy.on("error", (err: Error) => {
        console.error(
          `${LOG_PREFIX} Proxy error for ${routeWithSlash}: ${err.message}`,
        );
      });
      if (isVerbose) {
...
function buildProjectsProxy(): Record<string, ProxyOptions> {
  ...
  for (const p of projects) {
    ...
    const port = resolveProjectPort(p.id, portsMap);
    console.log(
      `${LOG_PREFIX}   ${route} -> http://localhost:${port} (source: ${source})`,
    );
  }
  return proxy;
}
